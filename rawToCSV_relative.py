...
from power_calculations import determineNoiseFloor, freeSpacePathGain, averagePowerTimeDomain, averagePowerThresholdTimeDomain, averagePowerThresholdFilterFreqDomainReal, calibrationCurve
...
#relativeDir = r'E:\OSM T+C\OSM Measurements\2018-05 Table Mountain CW PN Intercomparison\Day 1 2018-05-30\2018-05-30 Table Mountain System Through Calibration\2018-05-30-11-52-27 OSM T+C Measurement Data'
...
#relativeDir = r'E:\OSM T+C\OSM Measurements\2018-05 Table Mountain CW PN Intercomparison\Day 2 2018-05-31\2018-05-31 System Through Calibration\2018-05-31-11-40-11 OSM T+C Measurement Data'
...
#relativeDir = r'E:\OSM T+C\OSM Measurements\2018-06 Salt Lake City Utah\SL_day_1\Through Calibration\2018-06-11-12-04-07 OSM T+C Measurement Data'
#workingDirs.append(r'E:\OSM T+C\OSM Measurements\2018-06 Salt Lake City Utah\SL_day_1\Drive 1 Suburban West')
#workingDirs.append(r'E:\OSM T+C\OSM Measurements\2018-06 Salt Lake City Utah\SL_day_2\Drive 1')

#relativeDir = r'E:\OSM T+C\OSM Measurements\2018-06 Salt Lake City Utah\SL_day_4\Through Calibration\2018-06-14-09-47-37 OSM T+C Measurement Data'
#workingDirs.append(r'E:\OSM T+C\OSM Measurements\2018-06 Salt Lake City Utah\SL_day_3\Drive 1')
#workingDirs.append(r'E:\OSM T+C\OSM Measurements\2018-06 Salt Lake City Utah\SL_day_4\Drive 1')

relativeDir = r'E:\OSM T+C\OSM Measurements\2018-06 Salt Lake City Utah\SL_day_5\Through Calibration 30 s #1\2018-06-15-10-27-57 OSM T+C Measurement Data'
workingDirs.append(r'E:\OSM T+C\OSM Measurements\2018-06 Salt Lake City Utah\SL_day_5\Drive 1')

#%%############################################################################
# Calculate the energy in one period of the relative data set.
###############################################################################
os.chdir(relativeDir)
# This assumes one .json and one .bin file in relativeDir.
for file in os.listdir(relativeDir):
    if file.endswith('.json'):
        jsonFileRel = os.path.basename(file)
    if file.endswith('.bin'):
        binFileRel = os.path.basename(file)
        
with open(jsonFileRel, 'r') as j:
    dataRel = json.load(j)
with open(binFileRel, 'rb') as b:
    IQbinRel = np.fromfile(b)

#%% Calculate the energy in one PDP period.
IdataRel = IQbinRel[0::2]
QdataRel = IQbinRel[1::2]

magRel_V2 = IdataRel**2 + QdataRel**2
powerRel_W = magRel_V2/100.0
...
peakIndsRel = detect_peaks(powerRel_W, mph=noiseFloorRel_W, mpd=nPointsPDPRel-10, edge='rising')
nPDPsRel = len(peakIndsRel)

peakOffset = 0
for i in range(nPDPsRel-1):
    APDPRel_W += powerRel_W[peakIndsRel[i]-peakOffset:peakIndsRel[i]-peakOffset+nPointsPDPRel]
# Complete calculating the averaged PDP.
APDPRel_W /= nPDPsRel-1

power1PDPRel_W = np.sum([i for i in APDPRel_W if i >= noiseFloorRel_W])#/len(powerRel_W[peakIndsRel[1]:peakIndsRel[2]])
#power1PDPRel_W = np.sum(i for i in powerRel_W[peakIndsRel[1]:peakIndsRel[2]])
...
#                break
            ...
            # Override with a constant antenna gain for all elevation angles.
            gainTx_dBi = 9.0
            gainRx_dBi = 2.0
...
#            bandwidth_Hz = float(setupData['filterBandwidth_Hz'])
            ...
            powerAvg_W = np.sum([i for i in APDP_W if i >= noiseFloorAPDP_W])#/len(APDP_W)
#            powerAvg_W = np.sum(APDP_W)
...
#            powerReceived_dBm = calibrationCurve(powerAvg_dBm, float(data['frequency_tx_GHz']), float(data['nBits']), float(data['slideFactor']))
            powerReceived_dBm = powerAvg_dBm
...
#            pathGain_dB = -(float(data['power_tx_dBm']) - powerReceived_dBm + gainRx_dBi + gainTx_dBi - rxCableLoss_dB - txCableLoss_dB)
            pathGain_dB = 10.0*np.log10(powerAvg_W/power1PDPRel_W) - 93.17
